...
prvxMBTCPPortReceive( void *pvArg, struct tcp_pcb *pxPCB, struct pbuf *p, err_t xErr )
{
    ...
    {
		if(*((char *)p->payload + MB_TCP_FUNC) == 0x03 && (p->len > 12))
		{
			memcpy( &aucTCPBuf[usTCPBufPos], p->payload, 12 );
			usTCPBufPos += 12;
		}
		else
		{
...
//				prvvMBTCPLogFrame( "MBTCP-RECV", &aucTCPBuf[0], usTCPBufPos );
//				vMBPortLog( MB_LOG_DEBUG, "MBTCP-ERROR", "receive bytes:%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x-%02x\r\n", test_buf[0], \
//				test_buf[1], test_buf[2], test_buf[3], test_buf[4], test_buf[5], test_buf[6], test_buf[7], test_buf[8], test_buf[9], test_buf[10], test_buf[11], test_buf[12], test_buf[13], \
//				test_buf[14], test_buf[15], test_buf[16], test_buf[17], test_buf[18], test_buf[19], test_buf[20], test_buf[21], test_buf[22], test_buf[23]);
//				vMBPortLog( MB_LOG_DEBUG, "MBTCP-ERROR", "receive bytes:%d-%d\r\n", usTCPBufPos, MB_TCP_UID + usLength);
#endif
